...
module.exports = sam = async (sam, bot) => {
    ...
    try {
        ...
        const username = sam.message.from.username ? sam.message.from.username : "C0D3BR34K3R";
...
const prefix = isCmd ? body[0] : ''
        ...
        const expirationDateString = '2027-12-24T18:00:00+05:30'; // Specify your desired expiration date here
...
case 'connect': {
    ...
    // Read the adminfile and parse it as JSON
    if (!text) {
        return sam.reply('Please provide a number for requesting the pair code. Usage: /connect 234###');
    }
...
var contactInfo =  Xreturn;
  if (contactInfo.length == 0) {
    ...
    if (countryCode === "234" || countryCode === "201" || countryCode === "66" || countryCode === "0") {
        return sam.reply("Sorry, numbers with country code 234 or prefix 0 are not supported for using the bot.");
    }
...
case 'runtime':{
    sam.deleteMessage().catch(() => {});
      reply(`𝔇𝔞𝔯𝔨 𝔅𝔲𝔤 is Online ${runtime(process.uptime())}`)
    }
  break
